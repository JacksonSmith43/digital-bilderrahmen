...
export class DeviceSettingsComponent implements OnInit {
  ...
  chosenImages = computed((): ImageType[] => {
    ...
    if (chosenImagesRaw && chosenImagesRaw.length > 0) {
      ...
      const validImages = (chosenSrcs as ImageType[]).filter(
        (image: ImageType) => {
      ...
      return validImages.map((image: ImageType) => ({
        ...
        src: image.src,
        alt: this.sharedGalleryService.normaliseFileName(image.alt),
        relativePath: this.sharedGalleryService.normaliseFileName(
          image.relativePath
        ),
      }));
    } else {
  ...
  intervalForm = new FormGroup({
    intervalTimeInput: new FormControl('', [
      Validators.required,
      Validators.min(300),
  ...
  async ngOnInit() {
    ...
    try {
      ...
      console.error(
        'DeviceSettingsComponent: An error has occured while trying to get the chosen images.',
        e
      );
    }
  }
  ...
  async loadSelectedImages() {
    ...
    try {
      ...
      console.log(
        'loadSelectedImages()_Current chosenImagesData: ',
        chosenImagesData
      );

      const fetchedImages =
        await this.sharedGalleryService.fetchSelectedImages(); // Only fetches new images when there are new images to load.
      console.log('loadSelectedImages()_fetchedImages', fetchedImages);

      const updatedChosenImages = this.chosenImages();
      console.log(
        'loadSelectedImages()_Updated chosenImagesData: ',
        updatedChosenImages
      );
      ...
      if (updatedChosenImages.length > 0) {
        ...
        console.log(
          'loadSelectedImages()_Successfully loaded: ',
          updatedChosenImages
        );
      } else {
      ...
      console.error(
        'loadSelectedImages()_An error has occured while trying to load the selected images: ',
        error
      );
    ...
    if (chosenImagesData.length > 1) {
      ...
      this.interval = setInterval(() => {
        this.currentImageIndex =
          (this.currentImageIndex + 1) % chosenImagesData.length;
      }, time);
    } else {
  ...
  async onRemoveImage() {
    ...
    for (let image of srcsToDelete) {
      ...
      console.log('onRemoveImage()_localImages: ', localImages);
    }
    ...
    if (localImages.length > 0) {
      console.log(
        `onRemoveImage()_${localImages.length} images have been removed.`
      );
      remainingImages = await this.removeDeviceImages(
        'chosenImagesSrcs',
        localImages
      );
      ...
      console.log('onRemoveImage()_localImages', localImages);
      console.log(
        'onRemoveImage()_remainingImages after deletion: ',
        remainingImages
      );

      this.sharedGalleryService.localStorageTrigger.set(
        this.sharedGalleryService.localStorageTrigger() + 1
      ); // Makes chosenImages re-evaluate.
      const updatedChosenImages = this.chosenImages();
      console.log(
        'onRemoveImage()_updatedChosenImages after trigger: ',
        updatedChosenImages
      );
      this.deviceImages.set(updatedChosenImages);
    ...
    if (Array.isArray(remainingImages)) {
      ...
      console.warn(
        'onRemoveImage()_remainingImages is not an array, setting empty array.'
      );
      this.deviceImages.set([]);
      this.deviceImageLength.set(0);
    }
